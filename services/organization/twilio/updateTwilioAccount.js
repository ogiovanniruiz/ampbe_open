...
const updateTwilioAccount = async(orgID) => {
    try { 
        ...
        app[0].update({voiceUrl: voice_url})

        var phonenums = await subClient.incomingPhoneNumbers.list()

        for(var i = 0; i < phonenums.length; i++){
            await subClient.incomingPhoneNumbers(phonenums[i].sid).update({smsUrl: sms_url}).then(incoming_phone_number => {return incoming_phone_number}).catch(e => {return e.message});
        }
